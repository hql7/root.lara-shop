...
//            {type: 'date', required: true, message: '请选择结束时间', trigger: 'blur'}
    ...
    methods: {
      ...
      // 获取商品规格列表
      get_seles() {
        window.axiox.post(this.getApi('root/auth/groupbuy_list'),
          this.dafa({
            index: this.index,
      ...
      get_seles_goods() {
        ...
        axiox.post(this.getApi('root/auth/groupbuy_goods'),
          this.dafa({
      ...
      edit_seles(id) {
        ...
        this.antiShake.antiShake((resolve, reject) => {
          window.axiox.post(this.getApi('root/auth/groupbuy_edit'),
            `id=${this.id}`
          ).then(res => {
            resolve(res);
            if (res.data.code === 0) {
      ...
      del_seles(id) {
        this.$confirm('是否确认将删除此活动?', '提示', {
          ...
          this.antiShake.antiShake((resolve, reject) => {
            window.axiox.post(this.getApi('root/auth/groupbuy_del'),
              this.dafa({
                id_list: this.del_id
              })
            ).then(res => {
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            this.antiShake.antiShake((resolve, reject) => {
              axiox.post(this.getApi('root/auth/groupbuy_save'),
                this.dafa({
                  id: this.id,
